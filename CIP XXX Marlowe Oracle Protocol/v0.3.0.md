...
> Rephrase this: publishing of that data point using Marlowe contract and the data will be available on-chain for a predictable amount of time. It then can be used by other smart contracts.

We can also imagine different approaches to data locking on the chain where a particular party or group of parties can close the contract and remove the data point from the UTxO set.
...
We propose to enhance Marlowe validator so it uses the CIP-0069 and mints a unique thread token which is not only proof of execution but also proof of correctness of the initial state. The correctness of the state means that the choice map is empty at the beginning. That thread token will be burn at the end of the contract execution.
