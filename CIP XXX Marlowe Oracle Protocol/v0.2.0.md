...
The above pair - `Party` and `ChoiceName` is sufficient to specify an oracle request - it securely and uniquely identifies the oracle and the data requested. But this pair can be missing some details which form the full context for the request. In many cases the `ChoiceName` should be extended with additional information which can be used by the oracle and the user space tooling to fully describe a given request context:

* As mentioned already the value should be delivered as an `Integer` so in many cases the translation of the data point(s) into an `Integer` is required. We call this transformation a `resolution`. `Resolution` can be part of the data query itself or predefined by the oracle and specific to the data set.

* Using plain public key hashes for oracle identification forms a secure base line for the query context but it is not sufficient to discover in a decentralized environment the corresponding oracle easily. In many cases the `ChoiceName` should be extended with a human readable name of the data source. The connection between the public key hash and the oracle should be approved by the oracle itself preferably through some secure and standardized channel (e.g. a web page or an API endpoint).

* The query can be complex and not suitable for human consumption so the request could also associate a human readable description of the query provided by oracle and its resolution. This description can be used by the user space tooling to provide a human digestible form of the query and its resolution.

* Assessing validity of the whole request: `data query`, `resolution` and `data source` is critical for the user which participates in the contract. In order to do so the oracle should provide a way to verify that triple.

The `ChoiceName` can directly embed that set of the metadata or because of on chain space limitations provide an URI which can be used by the oracle and the user space tooling to fetch that set.

#### Query Structure

The metadata represented by the `ChoiceName` (directly or through reference IPFS / URI etc.) should be `UTF-8` encoded [CSV](https://www.rfc-editor.org/rfc/rfc4180) (Comma-separated values) string which describes a collection of values.

```
data_query,   data_source,      data_query_schema,    resolution...
ANY,          URI,              URI,                  (URI | JQ | ...)
```

* Only the **data_query** is required field and the **data_source** is strongly recommended.

* Any extra fields may be present, but the fields **data_source**, **data_query**, and **resolution** must be included. **API** can be skipped using empty value.

* All the values should be valid `URI`s. If the domain part remains consistent across them it could be only specified for the **data_source** and skipped
...
The **data_source** should be the root URL for the oracle service whose public key is in the `Party` component of the `ChoiceId` of which this `ChoiceName` is the other component. Please note that the `ChoiceName` gives a way to help identify the data source but it should not be trusted. The trust relies on the public key so the user of the protocol (with the help of official tooling) should confirm this correspondence.
...
The **data_query** should contain a query to the data\_source that conforms to the API.
...
##### Minimal Marlowe Request -> Refine this
