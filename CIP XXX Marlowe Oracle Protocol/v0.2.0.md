...
* As mentioned already the value should be delivered as an `Integer` so in many cases the translation of the data point(s) into an `Integer` is required. We call this transformation a `resolution`. `Resolution` can be part of the data query itself or predefined by the oracle and specific to the data endpoint.

* Using plain public key hashes for oracle identification forms a fully secure baseline but it is not sufficient to discover in a decentralized environment the corresponding oracle easily. In many cases the `ChoiceName` should be extended with a human readable name of the data source. The connection between the public key hash and the oracle should be presented by the oracle itself preferably through some secure and standardized channel (e.g. a web page or an API endpoint).

* The query can be complex and not suitable for human consumption so the request could also associate a human readable description of the query. Again this description should be provided by a trusted source to the end users through oracle service. This description can be used by the user space tooling to provide a human digestible form of the query and its resolution.

* When a `resolution` is predefined by the oracle then its description should be documented as part of the endpoint API. If custom `resolution` is used then the verification of it will be the responsibility of the user himself.
...
In both cases, the resolved content must conform to the CSV structure described below. References should be content-addressable to ensure immutability and tamper-resistance. Oracle providers and users are responsible for making referenced queries accessible and verifiable.
...
* The only required fields are `data_query`. All the other fields are optional but we **strongly recommend** to use all of them. A value which is not provided should be represented as an empty field in the CSV (`value1,,value3`).

* The `version` field should contain an integer which represents the version of the protocol used to encode the query. The current version is `1`. This field is used to allow future extensions of the protocol without breaking existing queries.

* The `data_query` should contain a query to the data source that conforms to the API. It can be a URL encoded query string or a properly CSV escaped JSON object string or CBOR hex value.
...
* This `Query` format (version 1) is fully extensible and any additional custom fields can be attached by the oracles and their users at the end of the query.
